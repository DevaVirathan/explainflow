...
def main():
    ...
    def run(
        ...
        theme: str = typer.Option(
            "dark", "--theme", "-t", help="Color theme (dark, light, colorblind)"
        ),
        max_steps: int = typer.Option(
            1000, "--max-steps", "-m", help="Maximum execution steps"
        ),
        fps: float = typer.Option(
            1.0, "--fps", help="Frames per second for GIF/video output"
        ),
        ...
        Examples:
            ...
            explainflow run script.py -o trace.mp4 --fps 1
            explainflow run script.py -o trace.md
            explainflow run script.py --heap --profile
        """
        ...
        if output:
            ...
            if suffix == ".png":
                ...
                export_video(trace, str(output), fps=fps, theme=theme)
                typer.echo(f"✅ Exported video to {output}")
            elif suffix == ".md":
                export_markdown(trace, str(output))
                typer.echo(f"✅ Exported Markdown to {output}")
            else:
