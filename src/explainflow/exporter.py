...
def export_image(
    ...
    if show_all_steps:
        ...
        height = (
            HEADER_HEIGHT
            + (num_code_lines * CODE_LINE_HEIGHT)
    ...
    try:
        ...
        try:
            # Try macOS fonts
            code_font = ImageFont.truetype(
                "/System/Library/Fonts/Menlo.ttc", CODE_FONT_SIZE
            )
            font = ImageFont.truetype(
            ...
            try:
                ...
                try:
                    ...
                    code_font = ImageFont.truetype(
                        os.path.join(fonts_dir, "consola.ttf"), CODE_FONT_SIZE
                    )
                    font = ImageFont.truetype(
                        os.path.join(fonts_dir, "arial.ttf"), DEFAULT_FONT_SIZE
                    )
                    header_font = ImageFont.truetype(
                        os.path.join(fonts_dir, "arialbd.ttf"), 18
                    )
                except OSError:
    ...
    draw.rectangle(
        [
            (PADDING, y_offset),
            (width - PADDING, y_offset + num_code_lines * CODE_LINE_HEIGHT + PADDING),
        ],
    ...
    for i, line in enumerate(code_lines, 1):
        ...
        if i == highlight_line:
            draw.rectangle(
                [
                    (PADDING + 1, line_y - 2),
                    (width - PADDING - 1, line_y + CODE_LINE_HEIGHT - 2),
                ],
        ...
        # Line number
        draw.text(
            (PADDING + 5, line_y), f"{i:3}", fill=colors["line_number"], font=code_font
        )

        # Code line
        draw.text(
            (PADDING + 50, line_y), line, fill=colors["foreground"], font=code_font
        )
    ...
    if current_step_data:
        _draw_step_info(
            draw, current_step_data, y_offset, width, colors, font, code_font
        )
        y_offset += 200
...
def _draw_step_info(
    draw, step, y_offset: int, width: int, colors: dict, font, code_font
) -> None:
...
def export_gif(
    ...
    for i in range(1, len(trace.steps) + 1):
        temp_filename = os.path.join(
            tempfile.gettempdir(), f"explainflow_frame_{i}.png"
        )
...
def export_video(
    ...
    try:
        ...
        raise ImportError(
            "Pillow is required for video export. Install with: pip install pillow"
        )
    ...
    for i in range(1, len(trace.steps) + 1):
        temp_filename = os.path.join(
            tempfile.gettempdir(), f"explainflow_video_frame_{i}.png"
        )
        export_image(trace, temp_filename, theme=theme, step=i, width=width)
    ...
    for step in trace.steps:
        ...
        if step.call_stack:
            ...
            for i, frame in enumerate(step.call_stack):
                ...
                lines.append(
                    f"- {indent}↳ **{frame.function_name}** (line {frame.line_number}){rv}"
                )
            lines.append("")
...
def export_html(
    ...
    for i, line in enumerate(code_lines, 1):
        ...
        code_lines_html.append(
            f'<div class="code-line" data-line="{i}"><span class="line-number">{i}</span><span class="line-content">{escaped_line}</span></div>'
        )
